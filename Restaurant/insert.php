...
if(isset($_POST['submit'])){
 ...
 $db = mysqli_select_db($conni,"sakshi");
 $time = $time . ":00";
 

 $Iquery = mysqli_query($conni,"select count(`RC_id`) from restaurant");
 $row = mysqli_fetch_assoc($Iquery);  #fetch result as an array
 $row_count = $row['count(`RC_id`)'];
 $r_id = $row_count + 1; #Increment rest cust id and inserting in table restaurent
